...
const productsSet = new Set( products.map( ele => ele.name ) );
const productsMap = new Map();
products.forEach( p => {
    productsMap.set( p.name , p.price );
});
...
function display( products ) {
    products.forEach( ( price , name ) => {
        ...
        product.querySelector('.name').textContent = name;
...
addButton.addEventListener( 'click' , _ => {
    const name = clearInput( productName.value );
    const price = clearInput( productPrice.value );
    if ( !name || !price ) {
        alert('برجاء إدخال جميع بيانات المنتج!');
        return;
    }
    if ( productsSet.has( name ) ) {
        alert('هذا المنتج موجود بالفعل');
    } else {
        const approvedProduct = new Map( [
            [ name , price ]
        ] );
        productsMap.set( name , price );
        productsSet.add( name );
        display( approvedProduct );
        totalcost( approvedProduct );
        resetInput();
    }
});
...
function editElement(element) {
    ...
    const newName = clearInput( productName.value ) ?? oldName;
    const newPrice = clearInput( productPrice.value ) ?? oldPrice;
    const existance = productsSet.has( newName ) && newName !== oldName;
    if ( existance ) {
        alert('هذا المنتج موجود بالفعل');
        return;
    }
    ...
    productsSet.delete( oldName );
    productsSet.add( newName );
    productsMap.delete( oldName );
    productsMap.set( newName , newPrice );
    if ( newPrice !== oldPrice ) {
        totalCost.textContent = Number( totalCost.textContent ) - Number( oldPrice ) + Number( newPrice );
    }
...
function totalcost( data ) {
    ...
    totalCost.textContent = Number( totalCost.textContent ) + total;
}
totalcost( productsMap )